      ...
      | None ->
          Logs.warn (fun f ->
              f "Couldn't find parent %d of %a" task.parent_id Task.Id.pp
                task.id)
      | Some p ->
          Logs.info (fun f -> f "Adding parent %a with %a" Task.Id.pp p.node.id Task.Id.pp task.id);
          let node = node p.children task in
          Hashtbl.add t.by_id (Task.Id.to_extern task.id) node;
          p.children := node :: !(p.children);
          invalidate t)
  | _ ->
      Logs.info (fun f -> f "Adding %a" Task.Id.pp_extern (Task.Id.to_extern task.id));
      ...
      | None -> (
          ...
          | Some child -> (
              let parent_already_has_child =
                List.find_opt
                  (fun c ->
                    Task.Id.to_extern c.node.Task.id = Task.Id.to_extern child.node.id)
                  !(parent.children)
              in
              ...
              | None ->
                  ...
                  Hashtbl.replace t.by_id (Task.Id.to_extern new_child.id)
                    new_child_node;
                  invalidate t
              | Some parent_child ->
